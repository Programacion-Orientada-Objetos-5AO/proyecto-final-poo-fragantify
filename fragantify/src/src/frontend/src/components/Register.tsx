...
export function Register({ onSuccess }: RegisterProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      await register({ username, password, verificacionPassword: verification });
      onSuccess?.();
    } catch (err) {
      console.error("Register failed", err);
      setError("No pudimos crear la cuenta. Verifica los datos e intenta nuevamente.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <section className="bg-[#0f172a] min-h-[calc(100vh-64px)] flex items-center py-16">
      <div className="max-w-md w-full mx-auto px-4">
        <Card className="bg-black border border-[#d4af37]/40 text-white shadow-2xl">
          <CardHeader>
            <CardTitle className="text-3xl font-serif text-[#d4af37]">Crea tu cuenta</CardTitle>
            <CardDescription className="text-gray-300">
              Registrate para guardar favoritos y recibir recomendaciones.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                <label htmlFor="verification" className="text-sm text-gray-200">
