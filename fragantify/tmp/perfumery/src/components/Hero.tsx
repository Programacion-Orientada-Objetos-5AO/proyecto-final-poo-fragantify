...
export function Hero() {
  return (
    <section id="home" className="relative min-h-screen flex items-center bg-gradient-to-br from-[#0f172a] via-black to-[#1e293b]">
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        <ImageWithFallback
          src="https://images.unsplash.com/photo-1594035910387-fea47794261f?ixlib=rb-4.0.3&auto=format&fit=crop&w=1600&q=80"
          alt="Luxury perfume background"
          className="w-full h-full object-cover opacity-15"
        />
        {/* Overlay degradado más oscuro y sofisticado */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif text-white drop-shadow-2xl leading-tight">
          ...
          <span className="text-[#d4af37] block mt-2">Perfumería</span>
        </h1>
        <p className="mt-4 sm:mt-6 text-base sm:text-lg text-gray-200 max-w-2xl mx-auto px-4">
          Explora fragancias exclusivas, creadas con pasión y elegancia.
        </p>
        <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 px-4">
          <Button className="bg-[#d4af37] text-black px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg rounded-lg hover:bg-[#fbbf24] transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
            Explorar Catálogo
          </Button>
